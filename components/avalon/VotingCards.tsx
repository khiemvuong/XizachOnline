...
export default function VotingCards({
  ...
  const SCENE_BASE_WIDTH = 1060;
  const sceneBaseHeight = isVoting ? 640 : 590;

  useEffect(() => {
    if (!showCards) return;

    const updateScale = () => {
      if (!sceneViewportRef.current) return;

      const rect = sceneViewportRef.current.getBoundingClientRect();
      const availableWidth = Math.max(280, rect.width - 10);
      const availableHeight = Math.max(220, rect.height - 10);

      const widthScale = availableWidth / SCENE_BASE_WIDTH;
      const heightScale = availableHeight / sceneBaseHeight;
      const nextScale = Math.min(1, widthScale, heightScale);

      setSceneScale(Math.max(0.36, nextScale));
    };

    updateScale();

    const observer = typeof ResizeObserver !== "undefined" ? new ResizeObserver(updateScale) : null;
    if (observer && sceneViewportRef.current) {
      observer.observe(sceneViewportRef.current);
    }
      ...
      return (
        <div className="absolute top-8 left-1/2 z-50 -translate-x-1/2 pointer-events-none">
          <div className="avalon-vote-wait-pill avalon-glass px-6 py-3 rounded-full border border-(--outline-variant)">
            <span className="avalon-vote-wait-text text-primary-avalon text-sm uppercase st font-bold">
  ...
  return (
    <div className="absolute inset-0 z-50 bg-black/65 backdrop-blur-sm animate-in fade-in duration-300 overflow-hidden">
      <div ref={sceneViewportRef} className="relative w-full h-full">
        <div
          className="absolute left-1/2 top-1/2 pointer-events-auto"
          style={{
            width: `${SCENE_BASE_WIDTH}px`,
            height: `${sceneBaseHeight}px`,
        ...
        >
          <div className="h-full w-full flex flex-col justify-center px-6">
            <div className="mb-6 text-center">
              <h2 className="font-headline text-4xl tracking-widest text-primary-avalon drop-shadow-[0_4px_4px_rgba(0,0,0,0.45)]">
                The Council Decides
              </h2>
            ...
            {isVoting && (
              <div className="mb-6 mx-auto w-215 rounded-xl border border-primary/25 bg-surface-container-low/70 px-4 py-3 text-center">
            ...
            <div className="flex gap-10 items-center justify-center">
              {isVoting ? (
                <>
                  <button
                    className="group relative w-full max-w-85 aspect-3/4 bg-surface-container-high rounded-xl overflow-hidden transition-all duration-500 hover:-translate-y-3 card-glow-primary border border-primary/15"
                  ...
                  <button
                    className="group relative w-full max-w-85 aspect-3/4 bg-surface-container-high rounded-xl overflow-hidden transition-all duration-500 hover:-translate-y-3 card-glow-tertiary border border-tertiary/15"
                ...
                <>
                  <button
                    className="group relative w-full max-w-85 aspect-3/4 border flex flex-col items-center justify-center transition-transform hover:-translate-y-3 shadow-2xl"
                    style={{
                  ...
                  <button
                    className={`group relative w-full max-w-85 aspect-3/4 border flex flex-col items-center justify-center shadow-2xl ${failCardLocked ? "cursor-not-allowed opacity-45 saturate-60" : "transition-transform hover:-translate-y-3 cursor-pointer"}`}
                    style={{
