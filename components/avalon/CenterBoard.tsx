...
export default function CenterBoard({
  ...
  return (
    <div
      className="relative flex flex-col items-center justify-between w-full max-w-[420px] rounded-3xl avalon-glass z-10 p-5 md:p-6 shadow-[0_20px_50px_rgba(0,0,0,0.6)] border border-white/5"
      style={{ 
        background: "linear-gradient(135deg, rgba(15,23,42,0.9) 0%, rgba(6,20,35,0.95) 100%)", 
    ...
    >
      ...
      <div className="z-10 w-full flex justify-between items-start gap-2 md:gap-4 px-2">
        {gameState.questHistory.map((q: any, idx: number) => {
           ...
           const isFuture = idx > gameState.currentQuestIndex;
           const questNumber = idx + 1;
           
           let cardContent;
           let cardStyles = "w-14 h-14 md:w-16 md:h-16 rounded-xl border flex flex-col items-center justify-center transition-all duration-500 shadow-lg";
           let labelContent = `${q.teamSize} Players`;

           if (q.status === 'success') {
              cardStyles += " bg-(--primary)/10 border-(--primary)/40 shadow-(--primary)/10";
              cardContent = <ShieldCheck className="w-5 h-5 md:w-8 md:h-8 text-(--primary)" />;
           } else if (q.status === 'fail') {
              cardStyles += " bg-(--tertiary)/10 border-(--tertiary)/40 shadow-(--tertiary)/10";
              cardContent = <XCircle className="w-5 h-5 md:w-8 md:h-8 text-(--tertiary)" />;
           } else if (isCurrent) {
              cardStyles += " bg-(--surface-container-highest) border-white shadow-white/20 scale-110 z-20 ring-1 ring-white/50";
              cardContent = <Swords className="w-5 h-5 md:w-8 md:h-8 text-white animate-pulse" />;
              labelContent = `MISSION ${questNumber}`;
           } else {
              cardStyles += " bg-slate-900/50 border-slate-800/80 opacity-40";
              cardContent = <Lock className="w-4 h-4 md:w-6 md:h-6 text-slate-500" />;
           }
           ...
           return (
             <div key={idx} className="flex flex-col items-center gap-2 group">
                ...
                <span className={`text-[8px] md:text-[10px] font-bold uppercase tracking-widest whitespace-nowrap transition-colors ${isCurrent ? 'text-white' : 'text-slate-500'}`}>
                   {labelContent}
                </span>
             </div>
           )
        ...
        {gameState.state === "TEAM_BUILDING" && (
          <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-2 duration-700">
            ...
            <p className="text-[7px] md:text-[10px] font-medium text-slate-400 mt-0.5 uppercase tracking-widest italic opacity-80">
              {isLeader ? "Chọn kỵ sĩ đi thực thi nhiệm vụ" : `Chờ Thủ Lĩnh ${gameState.players[gameState.leaderIndex]?.name} chọn người`}
            </p>
      ...
      <div className="z-10 w-full flex flex-col items-center">
        <span className="text-[8px] md:text-[10px] font-bold tracking-[0.2em] text-slate-500 uppercase mb-2 md:mb-3">Rejection Tracker</span>
        <div className="flex gap-3 md:gap-4 items-center justify-center">
          {[1, 2, 3, 4, 5].map((level, i) => {
            ...
            return (
              <div 
                key={level} 
                className={`w-3 h-3 md:w-3.5 md:h-3.5 rounded-full transition-all duration-700 ${
                  isCurrentTracker 
