...
export default function AvalonSidebar({
  me,
  socket,
  isHost,
  isSpectator,
  ...
  return (
    <div className="absolute top-4 right-4 z-50 flex flex-col items-center gap-3">
      ...
      {isLobby && (
        <button
          ...
          className="p-2.5 bg-black/40 backdrop-blur-md border border-(--primary)/30 rounded-full hover:bg-(--primary)/10 text-(--primary) hover:text-white transition-colors shadow-lg cursor-pointer shrink-0"
          title="Đổi tên"
        >
      ...
      <button
        ...
        className="p-2.5 bg-black/40 backdrop-blur-md border border-slate-600/40 rounded-full hover:bg-slate-700/60 text-slate-400 hover:text-white transition-colors shadow-lg cursor-pointer shrink-0"
        title={
      ...
      {!isLobby && !isGameOver && !isSpectator && (
        <button
          ...
          className={`p-2.5 bg-black/40 backdrop-blur-md rounded-full transition-colors shadow-lg cursor-pointer border shrink-0 ${
            isRoleHidden
      ...
      {!isLobby && !isGameOver && !isSpectator && (
        <button
          ...
          className="p-2.5 bg-black/40 backdrop-blur-md border border-(--tertiary)/30 rounded-full hover:bg-(--tertiary)/10 text-(--tertiary) hover:text-white transition-colors shadow-lg cursor-pointer shrink-0"
